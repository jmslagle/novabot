...
//    }
    ...
    public GeocodedLocation geocodedLocation(double lat, double lon) {
        ...
        try {
            ...
            novaBot.novabotLog.info(String.format("Exceeded daily limit with key %s, removing from rotation. Enable key again with !reload.", key));
            novaBot.config.getKeys().remove(key);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
