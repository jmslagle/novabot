...
//     -Added support for disabling logLevel property value caching. (see "printCacheLogLevel")
// ----------------------------------------------------------------------------
...
public class Print
{
    ...
    private static boolean          printCacheLogLevel      = true;
    private static int              printLogLevel           = LOG_UNDEFINED; // lazy init (cached)
    private static int              printLogHeaderLevel     = LOG_UNDEFINED; // lazy init (cached)
    private static int              printLogIncludeFrame    = -1;            // lazy init (cached)
    private static int              printLogIncludeDate     = -1;            // lazy init (cached)

    private static boolean          printLogFile_init       = false;         // volatile?
    private static File             printLogFile            = null;
    
    private static long             printRotateLogFileSize  = -1L;           // lazy init (cached)
    private static long             printRotateDelAgeSec    = -1L;           // lazy init (cached)

    /**
    *** Resets all cached print logging settings. Does not change or reset the output streams
    **/
    public static void resetVars()
    {
        // -- cached log-level vars
    ...
    **/
    ...
    {
        int oldInclFrame = Print.printLogIncludeFrame;
        Print.printLogIncludeFrame = inclFrameState;
        return oldInclFrame;
    }
    ...
    **/
    ...
    {
        int val = Print.printLogIncludeFrame;
        if (val < 0) {
            val = (RTConfig.getBoolean(RTKey.LOG_INCL_STACKFRAME,false) || Print.isDebugLoggingLevel())? 1 : 0;
            if (Print.printCacheLogLevel) {
                Print.printLogIncludeFrame = val; // -- cache property value
            }
        }
        return (val > 0);
    }
    ...
    **/
    ...
    {
        int val = Print.printLogIncludeDate;
        if (val < 0) {
            val = RTConfig.getBoolean(RTKey.LOG_INCL_DATE,false)? 1 : 0;
            if (Print.printCacheLogLevel) {
                Print.printLogIncludeDate = val; // -- cache property value
            }
        }
        return (val > 0);
    }
    ...
    **/
    ...
    /**
    *** Sets the logging level cache state.  If true, the value of the following
    *** properties will be cached (set only once on first property query):
    ***     - log.level
    ***     - log.level.header
    ***     - log.include.frame
    ***     - log.include.date
    **/
    public static void setCacheLogLevel(boolean cache)
    {
        Print.printCacheLogLevel = cache;
        if (!Print.printCacheLogLevel) {
            // -- reset cached property values to "UNDEFINED"
            Print.printLogLevel           = LOG_UNDEFINED;
            Print.printLogHeaderLevel     = LOG_UNDEFINED;
            Print.printLogIncludeFrame    = -1;
            Print.printLogIncludeDate     = -1;
    ...
    /**
    *** Gets the logging level cache state.
    **/
    public static boolean getCacheLogLevel()
    {
        return Print.printCacheLogLevel;
    }
    ...
    **/
    public static void setLogLevel(int level, boolean inclDate, boolean inclFrame)
    {
        ...
        Print.printLogIncludeDate  = inclDate? 1 : 0;
        Print.printLogIncludeFrame = inclFrame? 1 : 0;
    }
    ...
    **/
    ...
    {
        int val = Print.printLogLevel;
        if (val <= LOG_UNDEFINED) {
            val = Print.parseLogLevel(RTConfig.getString(RTKey.LOG_LEVEL,null), LOG_INFO);
            if (Print.printCacheLogLevel) {
                Print.printLogLevel = val; // -- cache property value
            }
        }
        return val;
    }
    ...
    **/
    public static void setLogHeaderLevel(int level)
    {
        ...
        Print.printLogHeaderLevel = level;
    }
    ...
    **/
    ...
    {
        int val = Print.printLogHeaderLevel;
        if (val <= LOG_UNDEFINED) {
            val = Print.parseLogLevel(RTConfig.getString(RTKey.LOG_LEVEL_HEADER,null), LOG_ALL);
            if (Print.printCacheLogLevel) {
                Print.printLogHeaderLevel = val; // -- cache property value
            }
        }
        return val;
    }
